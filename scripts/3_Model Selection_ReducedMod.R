...
SummaryData_Effort <- SampleData %>%
  ...
  split(.$Predictor) %>%
  purrr::map(~ as.list(.x)[-1])
# # Use this if needed to adjust to single net group values since three groups were deployed in each sample:
# SummaryData_Effort$Effort$`Mean_Centipede Net` <- SummaryData_Effort$Effort$`Mean_Centipede Net`/3
...
# if (run_distribution_tests) {
  ...
  #** Poisson has lowest AICc, followed by NBinom1 (delta = 2.42)*
...
p_abund <- ggplot(geardat, aes(x = Gear, y = Abundance)) +
  geom_point(shape = 16, color = "gray50", alpha = 0.5, position = position_nudge(x = -0.2)) +
  geom_boxplot(aes(fill = Gear), alpha = 0.75, width = 0.1, outlier.shape = NA) +
  geom_point(data = cld_abund, aes(y = response), size = 1, color = "red", position = position_nudge(x = 0.2)) +
  geom_errorbar(data = cld_abund, aes(y = NULL, ymin = asymp.LCL, ymax = asymp.UCL), width = 0.1, color = "red", position = position_nudge(x = 0.2)) +
  geom_text(data = cld_abund, aes(y = asymp.UCL, label = str_trim(.group)), 
            # size = 4, 
...
# Richness Plot
p_rich <- ggplot(geardat, aes(x = Gear, y = Richness)) +
  geom_point(shape = 16, color = "gray50", alpha = 0.5, position = position_nudge(x = -0.2)) +
  geom_boxplot(aes(fill = Gear), alpha = 0.75, width = 0.1, outlier.shape = NA) +
  geom_point(data = cld_rich, aes(y = rate), size = 1, color = "red", position = position_nudge(x = 0.2)) +
  geom_errorbar(data = cld_rich, aes(y = NULL, ymin = asymp.LCL, ymax = asymp.UCL), width = 0.1, color = "red", position = position_nudge(x = 0.2)) +
  geom_text(data = cld_rich, aes(y = asymp.UCL, label = str_trim(.group)), 
            # size = 4, 
            position = position_nudge(x = 0.3), hjust = 0) +
...
p_div <- ggplot(geardat, aes(x = Gear, y = Shannon)) +
  geom_point(shape = 16, color = "gray50", alpha = 0.5, position = position_nudge(x = -0.2)) +
  geom_boxplot(aes(fill = Gear), alpha = 0.75, width = 0.1, outlier.shape = NA) +
  geom_point(data = cld_div, aes(y = response), size = 1, color = "red", position = position_nudge(x = 0.2)) +
  geom_errorbar(data = cld_div, aes(y = NULL, ymin = asymp.LCL, ymax = asymp.UCL), width = 0.1, color = "red", position = position_nudge(x = 0.2)) +
  geom_text(data = cld_div, aes(y = response, label = str_trim(.group)), 
            # size = 4, 
            position = position_nudge(x = 0.3), hjust = 0) +
  scale_fill_manual(values = GearColors()) +
...
p_sim <- ggplot(geardat, aes(x = Gear, y = Simpson)) +
  geom_point(shape = 16, color = "gray50", alpha = 0.5, position = position_nudge(x = -0.2)) +
  geom_boxplot(aes(fill = Gear), alpha = 0.75, width = 0.1, outlier.shape = NA) +
  geom_point(data = cld_sim, aes(y = response), size = 1, color = "red", position = position_nudge(x = 0.2)) +
  geom_errorbar(data = cld_sim, aes(y = NULL, ymin = asymp.LCL, ymax = asymp.UCL), width = 0.1, color = "red", position = position_nudge(x = 0.2)) +
  geom_text(data = cld_sim, aes(y = response, label = str_trim(.group)), 
            # size = 4, 
            position = position_nudge(x = 0.3), hjust = 0) +
...
#   SimInt   = SimIntTop
# )
# save(geardat, ScaleData, AbundIntFit, RichIntFit, DivIntFit, SimIntFit, AbundIntTest, AbundIntTestAll, RichIntTest, RichIntTestAll, DivIntTest, SimIntTest, SimIntTestAll, TopAvgCoefs, AbundIntTable, RichIntTable, DivIntTable, SimIntTable, AvgMods, file = "output/models/DredgeListGLMM-ReducedMod.RData")
