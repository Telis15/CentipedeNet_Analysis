...
SiteData <- SiteData |>
  mutate(across(SecchiDepth_cm:Temperature_C, \(x) if_else(Gear != "Centipede Net", NA, x)))

# Add site-wide descriptors Dominant Substrate (descriptive), Mud Dominance (0,1) & Steepness (0,1,2)
SiteData <- SiteData |>
  left_join(
    wb_to_df("data/Sampling Data.xlsx", sheet = "Site_SubstrateSteepness"),
    by = join_by(Site)
  )
...
SiteData <- SiteData |>
  left_join(
    ...
    by = join_by(Date == date, Latitude == lat, Longitude == lon),
...
SiteData <- SiteData |>
  mutate(DaylightHrs = case_when(
    StartTime > Sunrise & EndTime < Sunset ~ as.numeric(difftime(EndTime, StartTime, units = "hours")),
    StartTime < Sunset & EndTime > Sunset ~ as.numeric(difftime(Sunset, StartTime, units = "hours")),
    StartTime < Sunrise & EndTime > Sunrise ~ as.numeric(difftime(EndTime, Sunrise, units = "hours")),
    .default = 0
  ), .after = EffortUnit) |>
...
SiteData <- SiteData |>
  left_join(wb_to_df("data/Sites_SlopeAspect.xlsx"), by = join_by(Site, Visit, Group, Latitude, Longitude)) |>
...
SiteData <- SiteData |>
  left_join(OcclDF[, c(1:5)], by = join_by(Site, Gear, Group)) |>
  relocate(c(SampleID, Site, Visit, Gear, Group))
rm(OcclDF)
...
MergedData <- full_join(
  FishData, 
  SiteData |>
    group_by(Site, Visit, Gear, Group) |>
    uncount(
      if_else(Gear == "Centipede Net" | Group == "0" | is.na(Group), 1, max(Effort, na.rm = TRUE)), 
      .remove = FALSE
    ) |>
    group_by(SampleID) |>
  ...
  mutate(Count = if_else(is.na(Species), 0, 1), .after = Species) |>
  relocate(c(SampleID, Site, Visit, Gear, Group)) |>
...
SampleData <- SiteData |>
  select(-c(Group)) |>
  filter(Effort > 0) |>
  mutate(Occlusion = mean(OcclusionGroupAvg, na.rm = TRUE), Occlusion_SD = sd(OcclusionGroupAvg, na.rm = TRUE), .by = Site) |>
...
Sample_Community_Matrix <- MergedData |>
  ...
  select(-any_of("NA")) |>  # Safely removes the NA species column if present
...
SampleData <- SampleData |>
  ...
  mutate(across(everything(), \(x) { x[is.infinite(x) | is.nan(x)] <- NA; x })) |>
...
save(MergedData, SiteData, SampleData, SpeciesList, FamilyList, Sample_Community_Matrix, Incidence_Matrices, GearSpeciesGrid, GearSpeciesCheck, file = "data/WrangledData.RData")